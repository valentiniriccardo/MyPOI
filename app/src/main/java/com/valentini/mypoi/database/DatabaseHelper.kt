...
open class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION){
    ...
    fun insertMarker (contentValues: ContentValues) : Int {
        ...
        val res: Cursor = db.rawQuery("SELECT $COL_ID FROM $TABLE_NAME WHERE $COL_NAME = '${contentValues.get(COL_NAME)}' AND $COL_LATITUDE = ${contentValues.get(
        COL_LATITUDE)} AND $COL_LONGITUDE = ${contentValues.get(
        COL_LONGITUDE)}", null)
        res.moveToFirst()
        return res.getInt(0)
    }
        ...
        while (!res.isAfterLast) {
            val tempmo =
                MarkerOptions()
                    ...
                    .snippet(res.getString(res.getColumnIndex(COL_ID)) + "#" + res.getString(res.getColumnIndex(COL_TYPE_NAME_COLOR)))
    ...
    {
        db = this.writableDatabase
        db.execSQL("UPDATE $TABLE_NAME SET $COL_LATITUDE = " + marker.position.latitude + ", $COL_LONGITUDE = " + marker.position.longitude + " WHERE $COL_ID = " + marker.snippet!!.substringBefore("#") + ";")
    }
